...
V10_DETECT_CLASS = (v10Detect, Detect_NMSFree, v10Detect_LSCD, v10Detect_SEAM, v10Detect_MultiSEAM)
...
OBB_CLASS = (OBB, OBB_LSCD, OBB_TADDH, OBB_LADH, OBB_LSCSBD, OBB_LSDECD)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C2f_IdentityFormerCGLU, C2f_RandomMixingCGLU, C2f_PoolingFormerCGLU, C2f_ConvFormerCGLU, C2f_CaFormerCGLU
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            c2 = m.channel
                   ...
                   DySample, CARAFE, CAA, ELA, CAFM, AFGCAttention, EUCB}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
        ...
        if isinstance(c2, list) and m not in {ChannelTransformer, PyramidContextExtraction, CrossLayerChannelAttention, CrossLayerSpatialAttention}:
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
